...
import type { VehicleType, Stop } from '@/types'
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="px-4 py-5 max-w-lg mx-auto space-y-5">
        ...
        <div className="space-y-3">
          ...
          <StopSearchInput onSelect={handleAddStop} />

          {stops.map((stop, i) => (
            <div key={stop.id} className="flex items-center gap-2 bg-white rounded-xl border border-slate-200 px-3 py-2.5">
              <span className="text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full bg-sky-100 text-sky-600 shrink-0">
                {i + 1}
              </span>
          ...
          <RouteMapPreview stops={stops} geometry={geometry} />
        </div>
