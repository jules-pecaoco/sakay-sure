...
export function driverDataLayer(
  base: TimeOfDayResult,
  activeDrivers: DriverRoute[],
  allDriverRoutes: DriverRoute[],
): DriverLayerResult {
  ...
  const etaMultiplier = effectiveCount > 0
    ? Math.max(0.5, 1 - effectiveCount * 0.08)
    : 1
